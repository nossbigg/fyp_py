...
from controller.tweet_llinks_utils import gen_represented_by_reverse_lookup_dict
...
class DataLabelService:
    ...
    def set_all_sentiments_collection(self, collection_name):
        ...
        tweet_links = {l["_id"]: l for l in list(cursor)}
        retweet_reverse_lookup_list = gen_represented_by_reverse_lookup_dict(tweet_links.values())

        # get cursor to label tweets
        unique_tweet_list = gen_unique_tweet_ids_from_links(tweet_links.values())
        ...
        for tweet in cursor:
            ...
            ids_to_update = [tweet_id]
            if tweet_id in retweet_reverse_lookup_list:
                reverse_lookup_id = retweet_reverse_lookup_list[tweet_id]
                childs = tweet_links[reverse_lookup_id]["childs"]
